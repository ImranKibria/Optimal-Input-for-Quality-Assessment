...
# INPUT PIPELINE
dataset_name = f'audio_feature_dataset/default'
...
def unpack_data(record):
  ...
  return normalization_layer(ftr_vec), mos
...
# LOAD OPTIMAL MODEL 
checkpoint_path = 'checkpoints/cp-0500.ckpt'
model = tf.keras.models.load_model(checkpoint_path)


# EVALUATE MODEL AT FRAME_LEVEL
...
for i in range(len(deg_files)):
  obsv_ds = chosen_ds.filter(lambda x: x['filename_deg'] == deg_files[i]).map(lambda y: tf.expand_dims(normalization_layer(y['feature_vector']), axis=0))
