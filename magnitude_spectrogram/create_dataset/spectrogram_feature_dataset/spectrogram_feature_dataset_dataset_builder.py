...
class Builder(tfds.core.GeneratorBasedBuilder):
  """DatasetBuilder for spectrogram_feature_dataset dataset."""
  ...
  def _generate_examples(self, split):
    ...
    for i, line in enumerate(df.itertuples()):
      ...
      ref_mag_spect, _ = self.get_stft(ref_wav, n_fft=n_fft, hop_len=hop_len, win_len=win_len, normalize=normalize)
      deg_mag_spect, _ = self.get_stft(deg_wav, n_fft=n_fft, hop_len=hop_len, win_len=win_len, normalize=normalize)

      tot_obsv, obsv_mat = self.get_obsv_mat(ref_mag_spect, deg_mag_spect, valid_frames)
